...
class MetricsHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        """Suppress stdlib HTTP access logging.

        EN: Keeps the exporter quiet on stdout/stderr during normal HTTP serving.
        RU: Не пишет access logs exporter'а в stdout/stderr при обычной HTTP-работе.
...
def main():
    ...
    print(f"listening on {scheme}://{args.listen_address}:{args.port}{args.metrics_path}")
